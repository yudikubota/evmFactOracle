...
describe("Subscription Oracle", function () {
  ...
  const _nodeId = 123;
  const _seed = 1568;
  let _license = 2; // subscription oracle
  ...
  it("Get Price, subscribe and get feed value in int and binary.", async function () {
    const { controllerContract: ctrlsc, controllerAddress, dataNodeContract, oracleContract, owner } = await loadFixture(deployOracle);
    await WarmupDataService(ctrlsc, dataNodeContract, oracleContract);

    const ownerAddress = await owner.getAddress();
    expect(await oracleContract.subscribe(ownerAddress, _feedId, { value: _price })).to.be.an.instanceOf(ethers.ContractTransactionResponse);
    // expect(await oracleContract.isFeedAvailable(2)).to.be.false;
    ...
    const ressubs = await oracleContract.checkSubscription(ownerAddress, _feedId);
    expect(ressubs[1]).be.equal(30);
